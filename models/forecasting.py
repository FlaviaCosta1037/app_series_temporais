...
def train_and_forecast(df):
    ...
    results["ARIMA"] = pd.DataFrame({"data": pd.date_range(df["data"].iloc[-1], periods=12, freq="M"), "yhat": forecast_arima})
    ...
    results["SARIMA"] = pd.DataFrame({"data": pd.date_range(df["data"].iloc[-1], periods=12, freq="M"), "yhat": forecast_sarima})
    ...
    results["SVR"] = pd.DataFrame({"data": pd.date_range(df["data"].iloc[-1], periods=12, freq="M"), "yhat": forecast_svr})
